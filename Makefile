...
check-git:
	...
	@if ! git remote -v | grep "upstream[[:space:]]git@github.com:ethereum/py-geth.git (push)\|upstream[[:space:]]https://github.com/ethereum/py-geth (push)"; then \
		echo "Error: You must have a remote named 'upstream' that points to 'py-geth'"; \
		exit 1; \
	fi
