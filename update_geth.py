...
with fileinput.FileInput("geth/install.py", inplace=True) as geth_install:
    latest_version_upper_underscored = LATEST_SUPPORTED_GETH_VERSION.upper().replace(
        ".", "_"
    )
    ...
    for line in geth_install:
        ...
        elif (f"{latest_version_upper_underscored}: {latest_version_install}") in line:
            print(
...
with fileinput.FileInput("README.md", inplace=True) as readme:
    ...
    for line in readme:
        print(
            line.replace(LATEST_SUPPORTED_GETH_VERSION, latest_user_provided_version),
            end="",
        )
